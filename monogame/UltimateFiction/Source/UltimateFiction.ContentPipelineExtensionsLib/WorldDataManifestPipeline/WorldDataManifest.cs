using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;

namespace UltimateFiction.ContentPipelineExtensionsLib.WorldDataManifestPipeline
{
	public class WorldDataManifest
	{
		public class MapCoord
		{
			public string Map { get; set; }
			public int XTile { get; set; }
			public int YTile { get; set; }

			public (string Map, (int XTile, int YTile) Coord) AsTuple => (this.Map, (this.XTile, this.YTile));
			public (int XTile, int YTile) Coord => (this.XTile, this.YTile);

			public MapCoord(string map, int xTile, int yTile)
			{
				this.Map = map;
				this.XTile = xTile;
				this.YTile = yTile;
			}

			public MapCoord((string Map, (int XTile, int YTile) Coord) value)
			{
				this.Map = value.Map;
				this.XTile = value.Coord.XTile;
				this.YTile = value.Coord.YTile;
			}

			public MapCoord(MapCoord value)
			{
				this.Map = value.Map;
				this.XTile = value.Coord.XTile;
				this.YTile = value.Coord.YTile;
			}

			public static MapCoord Parse(string text)
			{
				KeyValue(text, "(", out string map, out string coordsText);
				var coords = ParseCoord(coordsText);
				var result = new MapCoord(map, coords.XTile, coords.YTile);

				return result;
			}

			protected static (int XTile, int YTile) ParseCoord(string text)
			{
				string s = text
					.Replace(" ", "")
					.Replace("(", "")
					.Replace(")", "");
				KeyValue(s, ",", out string xtext, out string ytext);

				if (!int.TryParse(xtext, out int x))
					throw new Exception($"Error parsing x-coord in manifest: '{text}'");
				if (!int.TryParse(ytext, out int y))
					throw new Exception($"Error parsing y-coord in manifest: '{text}'");

				return (x, y);
			}
			public override string ToString()
			{
				return $"{Map}({XTile}, {YTile})";
			}
		}

		protected static string KeyOf(string text, string delimiter, string defaultValue = "")
		{
			string[] parts = text.Split(new string[] { delimiter }, StringSplitOptions.None);
			if (parts.Length < 1)
				return defaultValue;
			return parts[0];
		}

		protected static string ValueOf(string text, string delimiter, string defaultValue = "")
		{
			string[] parts = text.Split(new string[] { delimiter }, StringSplitOptions.None);
			if (parts.Length < 2)
				return defaultValue;
			return parts[1];
		}

		protected static (string key, string value) KeyValue(string text, string delimiter, out string key, out string value, string defaultValue = "", string defaultKey = "")
		{
			string[] parts = text.Split(new string[] { delimiter }, StringSplitOptions.None);

			key = defaultKey;
			value = defaultValue;

			if (parts.Length >= 1)
				key = parts[0];
			if (parts.Length >= 2)
				value = parts[1];

			return (key, value);
		}

		protected static string Unsurround(string text, string leftSurround, string rightSurround)
		{
			if (text.StartsWith(leftSurround) && text.EndsWith(rightSurround))
				return text.Substring(leftSurround.Length, text.Length - leftSurround.Length - rightSurround.Length);
			return text;
		}

		protected static string Unquote(string text)
		{
			return Unsurround(text, "\"", "\"");
		}

		//protected static (string Map, (int XTile, int YTile)) ParseMapCoord(string text)
		//{
		//	string map = KeyOf(text, "(").Trim();
		//	string coordsText = "(" + ValueOf(text, "(").Trim();
		//	var coords = ParseCoord(coordsText);
		//	return (map, (coords.XTile, coords.YTile));
		//}

		//protected static (int XTile, int YTile) ParseCoord(string text)
		//{
		//	string s = text
		//		.Replace(" ", "")
		//		.Replace("(", "")
		//		.Replace(")", "");
		//	string xtext = KeyOf(s, ",");
		//	string ytext = ValueOf(s, ",");

		//	if (!int.TryParse(xtext, out int x))
		//		throw new Exception($"Error parsing x-coord in manifest: '{text}'");
		//	if (!int.TryParse(ytext, out int y))
		//		throw new Exception($"Error parsing y-coord in manifest: '{text}'");

		//	return (x, y);
		//}

		public class Teleport
		{
			public Teleport() { }
			public Teleport(MapCoord sourceMapCoord, MapCoord targetMapCoord)
			{
				this.SourceMapCoord = sourceMapCoord;
				this.TargetMapCoord= targetMapCoord;
			}

			public MapCoord SourceMapCoord { get; set; }
			public MapCoord TargetMapCoord { get; set; }
			public static Teleport Parse(string text)
			{
				KeyValue(text, ":", out string sourceText, out string targetText);

				Teleport result = new Teleport();
				result.SourceMapCoord = MapCoord.Parse(sourceText);
				result.TargetMapCoord = MapCoord.Parse(targetText);
				return result;
			}

			public override string ToString()
			{
				return $"{SourceMapCoord}:{TargetMapCoord}";
			}
		}

		public class NpcInteraction
		{
			public NpcInteraction() { }
			public NpcInteraction(MapCoord mapCoord, int spriteRow, int spriteCol, int spriteFrameCount, string dialogText)
			{
				this.MapCoord = mapCoord;
				this.SpriteRow = spriteRow;
				this.SpriteCol = spriteCol;
				this.SpriteFrameCount = spriteFrameCount;
				this.DialogText = dialogText;
			}

			public MapCoord MapCoord { get; set; }
			public int SpriteRow { get; set; }
			public int SpriteCol { get; set; }
			public int SpriteFrameCount { get; set; }
			public string DialogText { get; set; }
			public static NpcInteraction Parse(string text)
			{
				KeyValue(text, ":", out string mapCoordText, out string theRestText);
				KeyValue(theRestText, ";", out string spriteDataText, out string dialogText);

				var spriteParts = spriteDataText.Split(",", StringSplitOptions.None).Select(s => s.Trim()).ToList();
				if (spriteParts.Count != 3)
					throw new Exception($"Error parsing NPC Interaction: {text}");
				int spriteRow = int.Parse(spriteParts[0]);
				int spriteCol = int.Parse(spriteParts[1]);
				int spriteFrameCount = int.Parse(spriteParts[2]);

				var result = new NpcInteraction(
					MapCoord.Parse(mapCoordText),
					spriteRow,
					spriteCol,
					spriteFrameCount,
					Unquote(dialogText.Trim()));
				return result;
			}

			public override string ToString()
			{
				return $"{MapCoord}: {SpriteRow}, {SpriteCol}, {SpriteFrameCount}; \"{DialogText}\"";
			}
		}

		public class Port
		{
			public Port() { }
			public Port(MapCoord mapCoord)
			{
				this.MapCoord = mapCoord;
			}

			public MapCoord MapCoord { get; set; }
			public static Port Parse(string text)
			{
				return new Port(MapCoord.Parse(text));
			}

			public override string ToString()
			{
				return MapCoord.ToString();
			}
		}

		public class Shop
		{
			public Shop() { }
			public Shop(MapCoord mapCoord, string name)
			{
				this.MapCoord = mapCoord;
				this.ShopName = name;
				this.Type = ShopName.Split('_').Last();
			}

			public MapCoord MapCoord { get; set; }
			public string ShopName;
			public string Type;

			public static Shop Parse(string text)
			{
				KeyValue(text, ":", out string mapCoordText, out string shopNameText);
				Shop result = new Shop(MapCoord.Parse(mapCoordText), Unquote(shopNameText.Trim()));
				return result;
			}

			public override string ToString()
			{
				return $"{MapCoord}: {ShopName}";
			}
		}

		public class Trigger
		{
			public Trigger() { }
			public Trigger(MapCoord mapCoord, string actionName)
			{
				this.MapCoord = mapCoord;
				this.ActionName = actionName;
			}

			public MapCoord MapCoord { get; set; }
			public string ActionName { get; set; }

			public static Trigger Parse(string text)
			{
				KeyValue(text, ":", out string mapCoordText, out string actionName);
				Trigger result = new Trigger(MapCoord.Parse(mapCoordText), actionName.Trim());
				return result;
			}

			public override string ToString()
			{
				return $"{MapCoord}: {ActionName}";
			}
		}

		public class Treasure
		{
			public Treasure() { }
			public Treasure(MapCoord mapCoord, string treasureName, int quantity)
			{
				this.MapCoord = mapCoord;
				this.TreasureName = treasureName;
				this.Quantity = quantity;
			}

			public MapCoord MapCoord { get; set; }
			public string TreasureName { get; set; }
			public int Quantity { get; set; }
			public static Treasure Parse(string text)
			{
				KeyValue(text, ":", out string mapCoordText, out string theRestText);
				KeyValue(theRestText, ",", out string treasureName, out string quantityText);

				MapCoord mapCoord = MapCoord.Parse(mapCoordText);
				int quantity = int.Parse(quantityText);

				Treasure result = new Treasure(mapCoord, treasureName, quantity);
				return result;
			}

			public override string ToString()
			{
				return $"{MapCoord}: {TreasureName}, {Quantity}";
			}
		}

		public class Battle
		{
			public Battle() { }
			public Battle(MapCoord mapCoord, string battleName)
			{
				this.MapCoord = mapCoord;
				this.BattleName = battleName;
			}

			public MapCoord MapCoord { get; set; }
			public string BattleName { get; set; }

			public static Battle Parse(string text)
			{
				KeyValue(text, ":", out string mapCoordText, out string battleName);
				Battle result = new Battle(MapCoord.Parse(mapCoordText), Unquote(battleName.Trim()));
				return result;
			}

			public override string ToString()
			{
				return $"{MapCoord}: {BattleName}";
			}

		}

		public class ResumePoint
		{
			public ResumePoint() { }
			public ResumePoint(MapCoord mapCoord, string innName)
			{
				this.MapCoord = mapCoord;
				this.InnName = innName;
			}
			public MapCoord MapCoord { get; set; }
			public string InnName { get; set; }
			public static ResumePoint Parse(string text)
			{
				KeyValue(text, ":", out string innName, out string mapCoordText);
				ResumePoint result = new ResumePoint(MapCoord.Parse(mapCoordText), Unquote(innName.Trim()));
				return result;
			}

			public override string ToString()
			{
				return $"{InnName}: {MapCoord}";
			}
		}

		public List<Teleport> Teleports { get; }
		public List<NpcInteraction> NpcInteractions { get; }
		public List<Port> Ports { get; }
		public List<Shop> Shops { get; }
		public List<Trigger> Triggers { get; }
		public List<Treasure> Treasures { get; }
		public List<Battle> Battles { get; }
		public List<ResumePoint> ResumePoints { get; }

		public WorldDataManifest()
		{
			Teleports = new List<Teleport>();
			NpcInteractions = new List<NpcInteraction>();
			Ports = new List<Port>();
			Shops = new List<Shop>();
			Triggers = new List<Trigger>();
			Treasures = new List<Treasure>();
			Battles = new List<Battle>();
			ResumePoints = new List<ResumePoint>();
		}

	}
}
